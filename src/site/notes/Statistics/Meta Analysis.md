...
### Graphical Representation: The [[Statistics/Forest Plot\|Forest Plot]]

- A [[Statistics/Forest Plot\|Forest plot]] provides a graphical illustration of the meta-analysis results, displaying individual study estimates alongside the combined summary estimate.
...
- A diamond at the bottom represents the overall pooled summary estimate, with its horizontal tips indicating the final 95% [[Statistics/Confidence Interval\|confidence interval]]; if the tips cross the line of null effect, the combined result is not statistically significant.
...
- **[[Statistics/Funnel Plot\|Funnel Plot]]:** An informal scatterplot assessing publication bias by plotting treatment effect sizes against measures of precision (such as sample size or standard error).
...
- **Egger’s and Begg’s Tests:** Formal statistical tests utilized to analyze [[Statistics/Funnel Plot\|funnel plot]] asymmetry; Egger's test uses linear regression of effect size against precision to detect bias (significant if p < 0.05).
...
- **[[Statistics/Network Meta-Analysis\|Network Meta-analysis]]:** Summarizes both direct and indirect evidence across multiple treatments for a given condition, useful when interventions differ across individual trials.
